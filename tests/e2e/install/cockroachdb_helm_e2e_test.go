...
func (h *CockroachDBHelm) TestHelmMigration(t *testing.T) {
	...
	publicService := fmt.Sprintf("%s-cockroachdb-public", helmchart.ReleaseName)
	...
	hosts := []string{
		...
		fmt.Sprintf("*.%s", fmt.Sprintf("%s-cockroachdb", helmchart.ReleaseName)),
		fmt.Sprintf("*.%s.%s", fmt.Sprintf("%s-cockroachdb", helmchart.ReleaseName), h.Namespace),
		fmt.Sprintf("*.%s.%s.svc.%s", fmt.Sprintf("%s-cockroachdb", helmchart.ReleaseName), h.Namespace, "cluster.local"),
	}
