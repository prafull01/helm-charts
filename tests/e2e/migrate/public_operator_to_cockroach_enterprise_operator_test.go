...
func (o *PublicOperatorToCockroachEnterpriseOperator) TestDefaultMigration(t *testing.T) {
	...
	verifyOperatorMigration(t, o.CustomResourceBuilder.Cr())

	testutil.TestIngressRoutingDirect(t, "ui.local.com")
}

// verifyPriorityClassMigration verifies that the priority class has been correctly migrated
// from the original CrdbCluster to the running pods
func verifyOperatorMigration(t *testing.T, crdbCluster *api.CrdbCluster) {
	kubectlOptions := k8s.NewKubectlOptions("", "", crdbCluster.Namespace)

	// Get all pods for this cluster
	pods := k8s.ListPods(t, kubectlOptions, metav1.ListOptions{
		LabelSelector: fmt.Sprintf("app=%s", crdbCluster.Name),
	})
