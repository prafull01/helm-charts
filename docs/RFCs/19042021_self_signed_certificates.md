...
* Check if CA secret is empty
  * if yes, [Generate-CA](####Generate-CA), leading to generation of node and client certificates; return
  * if not, [Validate Secret Annotations for CA](####Validate-Secret-Annotations)
    * if valid, CA is intact
    * if not, [Annotate-CA](####Annotate-CA), leading to generation of node and client certificates; return
...
* Check if node or client certificates [needs to be regenerated](####Check-cert-for-regeneration):
    * if yes, follow [Node cert generation workflow](####Generate-Node-Cert)
