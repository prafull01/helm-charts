...
kubectl -n $NAMESPACE get secret -o yaml $crdb_name-ca-secret | yq '.data."ca.key"' | base64 -d >certs/ca.key
kubectl -n $NAMESPACE get secret -o yaml $crdb_name-ca-secret | yq '.data."ca.crt"' | base64 -d >certs/ca.crt
kubectl -n $NAMESPACE create configmap $crdb_name-ca --from-file=certs/ca.crt --dry-run=client -o yaml |
...
# operator.
kubectl -n $NAMESPACE get secret -o yaml $crdb_name-node-secret >backup/cockroachdb-node-secret.yaml
...
kubectl -n $NAMESPACE create secret generic $crdb_name-node-secret --from-file=tls.crt=certs/node.crt --from-file=tls.key=certs/node.key --dry-run=client -o yaml |
  kubectl -n $NAMESPACE apply -f -
...
kubectl -n $NAMESPACE create secret generic $crdb_name-client-secret --from-file=tls.crt=./certs/client.root.crt --from-file=tls.key=./certs/client.root.key --dry-run=client -o yaml |
  kubectl -n $NAMESPACE apply -f -
